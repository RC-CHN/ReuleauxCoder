...
`app/configuration.py` owns the shared configuration management service. Model tools, `config.*` RPC methods and `rcoder config` use its candidate/validation/commit/recovery contracts. Persistent changes activate on the next start; inspection distinguishes them from the running configuration. The standalone `rcoder config rpc` transport starts without an Agent or valid model configuration. Typed schema and pure validation live in `services/config`; journaled atomic replacement lives in `infrastructure/persistence/config_transactions.py`. App defaults and named profiles resolve identically for startup, session switching and probes. Validation binds results to every affected profile; effective automatic reviewers and process-launch fields remain human-owned. VS Code recovery starts the original workspace-host command with `--config-management-stdio`, independent of Agent startup, and shares dirty-editor/revision guards. See `docs/configuration-api.md` for authorization, probe limits and recovery semantics, and `docs/configuration-reference.md` for the complete YAML inventory.
