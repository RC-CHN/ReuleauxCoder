...
`extensions/skills/builtin/` contains 17 self-contained English skills, including configuration, development/writing workflows and PPTX/DOCX/XLSX/PDF. Discovery precedence is builtin < user < project; disabled names apply to the resolved winner. Presets are read in place, never copied over user files. The catalog includes core Python/workspace/explicit-config launch paths so skills use the owning core instead of an unrelated PATH installation. The self-configuration skill owns the canonical field reference and reasoning/workflow guides. Office helpers use task-local optional libraries; the core has no Office-library runtime dependency. The `skill-tests` dependency group exercises real documents and script entry points on Linux/Windows CI. Tests compare configuration field coverage, validate resource closure, exercise recipes and verify all wheel/sdist resources and installed use outside the checkout. See `docs/builtin-skills.md`.
