...
本文随 rcoder 核心和 `rcoder-config` skill 分发，描述该版本的配置行为。先按 [SKILL.md](../SKILL.md) 确认核心所在主机、Python 和配置层，再调用 `config describe --section models`（或对应 `config.describe` RPC）核对字段。本文补充 schema 不能表达的加载规则、用途和限制；推理相关设置另见 [reasoning.md](reasoning.md)，可执行的检查流程与配置片段见 [workflows.md](workflows.md)。
...
| `/models/profiles/{name}/preserve_reasoning_content` | boolean / `true` | 收集、流式发送及保存提供方返回的可见推理文本；不是计算开关，也不控制 Responses 私有加密回放项 |
...
| `/models/profiles/{name}/reasoning_replay_mode` | string? / `null` | `null` / `none` 不强制补位，不代表删除所有推理；`tool_calls` 配合 preserve 为工具轮缺失字段补位，详见 reasoning.md |
...
`app` 的模型字段是共享默认值。先按文件层级合并，再把 app 默认值与各 profile 递归合并，最后填充类型默认值。profile 省略字段时继承 app；显式 `null` 清除可空字段的继承（例如 request_mode 回到提供方默认协议）。没有 profile 时，旧式 app 配置迁移为 `default` profile。启动、会话切换、子代理与验证使用同一份解析结果。修改 app 默认值会影响所有继承该字段的 profile，包括审批模型；检查 `inspect.model_targets` 的角色和解析结果。

| 路径 | 类型 / 缺省行为 | 说明 |
| --- | --- | --- |
| `/app/model` | string / 迁移时 `gpt-4o` | 旧式模型 ID |
| `/app/api_key` | string / 迁移时空 | 共享凭证；inspect 脱敏，普通文件仍包含原文，勿将脱敏值写回 |
...
扫描顺序为随包内置 → 用户目录 → 工作区目录，后者覆盖解析后同名的前者。内置 `rcoder-config` 随核心更新，不复制到用户目录；关闭用户/项目扫描不关闭内置项，用 `skills.disabled: [rcoder-config]` 单独禁用，`skills.enabled: false` 关闭全部。没有 `skills.paths`、远程 skill URL、marketplace 或可配置的 builtin 包目录。`SKILL.md` 要有 YAML frontmatter 的 `name` 和 `description`，正文按 skill 机制读取。`agents/openai.yaml` 不是 rcoder 当前 parser 的输入，不应直接把 Codex 安装结构当成 rcoder 实现。
...
| 路径 | 类型 / 默认值 | 用途 |
| --- | --- | --- |
| `/meta` | object / `{}` | 开放的 JSON 元数据对象；没有固定子字段集合 |

执行代码已知使用 `meta.example`（示例模板标记）；旧 `meta.workspace_bootstrapped` 标记保留但不再触发自动回填。所有现有配置都仍标记 example 时，普通启动会要求用户完成配置。不要为“让检查变绿”自动删除用户尚未完成配置的标记。
...
需要排查实现时，在当前核心安装包的 `reuleauxcoder/` 下查阅：`domain/config/models.py`（类型/校验）、`domain/config/schema.py`（默认规则）、`services/config/{definition,loader,validation,probe}.py`（schema、合并和检查）、`compat/config_migration.py`（迁移）、`services/llm/{request_parameters,providers,sanitizer,client}.py`（模型请求/回放）、`app/configuration.py`（只读接口）。这些是实现位置，不要求用户安装源码仓库。
