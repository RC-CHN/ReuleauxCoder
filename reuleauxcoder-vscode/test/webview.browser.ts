...
test('actual bilingual webview: immediate steering, retry, paste/upload and narrow layout', {timeout: 60000}, async () => {
  ...
  try {
    for (const language of ['en', 'zh']) {
      ...
      state.recovery = {busy: false, valid: false, diagnostics: [{code: 'invalid_yaml', path: '/', source: '/workspace/中文.yaml', line: 3, message: 'Invalid YAML', severity: 'error'}], sources: [{scope: 'explicit', path: '/workspace/中文.yaml', exists: true}], history: [{id: 'history-1', date: '2026-09-25T00:00:00Z', path: '/workspace/中文.yaml', scope: 'explicit'}]};
      await publish();
      const recovery = page.locator('#configuration-recovery');
      await recovery.waitFor({state: 'visible'});
      assert.equal(await recovery.evaluate(node => getComputedStyle(node).animationName), 'feedback-enter');
      assert.equal(await recovery.locator('h3').textContent(), language === 'zh' ? '配置恢复' : 'Configuration recovery');
      await recovery.locator('.recovery-source button').click();
      assert.equal(messages.at(-1)!.data!.scope, 'explicit');
      await recovery.locator('.recovery-history button').click();
      assert.equal(messages.at(-1)!.data!.id, 'history-1');
      state.recovery.candidate = {id: 'candidate-1', scope: 'explicit', target_path: '/workspace/中文.yaml', status: 'prepared', base_revision: 'rev', created_at: '2026-09-25T00:00:00Z', expires_at: 1, activation: 'next_start', diff: [{path: '/app/model', before: '<script>throw new Error("unsafe")</script>', after: 'restored-model', removed: false}], diagnostics: [], checks: [], requires_model_probe: true, model_preview: null, model_targets: [{profile: 'sub', roles: ['subagent'], fingerprint: 'fp', model: 'test', provider: 'openai-compatible', request_mode: 'chat-completions', base_url: null, max_tokens: 1024}]};
      state.recovery.validation = {change_id: 'candidate-1', valid: true, diagnostics: [], checks: [{check: 'static', status: 'passed', code: 'ok'}, {check: 'startup', status: 'passed', code: 'ok'}, {check: 'model', profile: 'sub', fingerprint: 'fp', status: 'unknown', code: 'timeout'}]};
      await publish();
      const restore = recovery.locator('button.primary');
      await restore.waitFor({state: 'visible'}); assert(await restore.isDisabled());
      assert.equal(await recovery.locator('script').count(), 0);
      assert((await recovery.locator('.recovery-before').textContent())?.includes('<script>'));
      const offline = recovery.locator('input[type="checkbox"]');
      await offline.check(); assert(await restore.isEnabled());
      await offline.uncheck(); assert(await restore.isDisabled());
      await offline.check(); await restore.click();
      assert.equal(messages.at(-1)!.action, 'configuration.apply');
      assert.equal(messages.at(-1)!.data!.id, 'candidate-1'); assert.equal(messages.at(-1)!.data!.offline, true);
      state.recovery.busy = true; await publish();
      await page.waitForFunction(() => document.querySelector<HTMLButtonElement>('#setup [data-command="start"]')!.disabled);
      assert(await restore.isDisabled()); assert(await offline.isDisabled());
      assert.equal(await page.locator('.recovery-progress').evaluate(node => getComputedStyle(node, '::before').animationName), 'working-pulse');
      // Motion never shifts a confirmation target; reduced motion removes loops and transitions.
      const decisionPosition = await restore.boundingBox();
      await page.emulateMedia({reducedMotion: 'reduce'});
      assert.equal(await recovery.evaluate(node => getComputedStyle(node).animationName), 'none');
      assert.equal(await page.locator('.recovery-progress').evaluate(node => getComputedStyle(node, '::before').animationName), 'none');
      assert.equal(await restore.evaluate(node => getComputedStyle(node).transitionDuration), '0s');
      assert.deepEqual(await restore.boundingBox(), decisionPosition);
      await page.emulateMedia({reducedMotion: 'no-preference'});
      state.recovery.busy = false; state.recovery.candidate.id = 'candidate-2'; await publish();
      await page.waitForFunction(() => !document.querySelector<HTMLInputElement>('.recovery-offline input')!.checked);
