...
function render(state: HostSnapshot): void {
  ...
  setup.querySelectorAll<HTMLButtonElement>('[data-command]').forEach(button => {button.disabled = (['starting', 'stopping', 'installing'].includes(state.phase) || !!state.configuration?.busy) && button.dataset.command !== 'logs';});
  ...
  setup.querySelector<HTMLButtonElement>('[data-command="configuration.open"]')!.hidden = needsInstall || !!state.configuration;
  setup.classList.toggle('configuring', !!state.configuration);
  document.body.classList.toggle('configuring', !!state.configuration);
  if (state.configuration) notice('');
