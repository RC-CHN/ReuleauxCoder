...
4. 点击 **检查配置**，或从输入框菜单进入 **配置文件**。默认展示“仅当前工作区”，全局默认值位于“全局默认设置 · 所有项目”；专用启动文件（`--config`）另行标出最高优先级。工作区没设置的项继承全局，全局属于核心主机（Remote 时为远端用户）。在原生编辑器创建/编辑文件，检查包含未保存内容；通过后保存，再单独点击 **启动核心／重启并生效**。修改不会自动重启；重启前检查失败会保留当前核心。可选模型连接测试不阻止保存。编辑沿用原生注释/格式、撤销和外部文件冲突处理。配置检查进程不依赖 Agent，旧核心缺少只读 API v2 时会提示更新。
...
- 补充要求默认在当前步骤后应用；等待时输入框上方出现 **立即引导**，可中断当前步骤、提前应用已发送的要求并继续任务，不暂停目标。重复点击不会变成停止；停止按钮单独保留。旧核心不支持此能力时会提示更新。顶部以文字和轻量动画区分思考、回复、执行与等待操作，尊重系统减少动态效果设置。
- 连续工具调用默认折叠为摘要，点击展开调用参数、格式化输出和推理；失败项自动展开一次。全局展开按钮可查看所有调用，连续进程轮询也保留每一次结果。手动展开状态在后续输出中保持；向上阅读时暂停自动滚动，滚回底部或点击 **新输出** 恢复跟随。
...
流式回归覆盖边输出边向上阅读、回到底部、已完成 Markdown 与增长中代码块的选区/复制/水平滚动，以及草稿图片焦点。运行 `node --import tsx scripts/profile-webview.ts /tmp/webview-profile.json` 可测量 240 个收起/展开工具和 2,400 行代码块在每 32ms 更新时的帧间隔、长任务、布局耗时与 DOM 变化；先执行 `npm run build`，浏览器路径沿用上述环境变量。基于本机 Chromium 的测量，修复前后长 Markdown 会话的帧间隔 P95 从约 50ms 降至约 16.8ms；长代码块 100 次更新的节点增删从 400 降至 3。数字用于比较同机实现，不能替代实际 Remote 网络、VS Code 合成器与目标设备测试。
...
While a task runs, new messages apply after the current step. **Guide now** appears beside queued messages to interrupt that step and apply guidance without stopping the task or pausing its goal. Repeated clicks remain safe; Stop is separate. Older cores show an update hint. Thinking, responding, running and waiting have distinct labels and subtle motion, with reduced-motion support. Process and goal clocks advance locally without rebuilding controls, including command panels, and stop on exit, pause or disconnection.

Consecutive tool calls collapse into an inspectable summary. Expand a call for arguments, formatted output and reasoning, or use the global disclosure control for all details. Failures open once; manual choices survive streaming. Repeated process checks retain every result. Scrolling upward pauses following; scroll to the bottom or select **New output** to resume. Git counts, statuses and diagnostic links use theme colors for easier inspection.
...
Open **Reuleaux → Conversation** to connect automatically. If the core is missing or incompatible, choose **Install compatible core** to install the checksum-verified bundled wheel in private extension storage using uv or venv/pip. Open **Configuration files** from the composer menu or **Check configuration** on startup. Edit the host's global defaults or the current workspace override in the native editor; explicit launch files have highest priority. Global defaults belong to the workspace host, including Remote. Checks include dirty buffers and never write files. Save, then explicitly start/restart; failed preflight keeps the running core alive. Model connection tests are optional and do not gate saving. File creation is explicit, and editing uses native undo and conflict handling. An older core without inspection API v2 prompts an update. The settings table above supports explicit executable, arguments and Python selection. No shell command interpolation is used.
...
Enter shows ordinary/steering input immediately and preserves the next draft. Files and clipboard images transfer as bounded byte chunks to the workspace host; sending during an upload waits for that message's attachments in the background. Image thumbnails appear inside the message and survive session restoration. Click to preview the sent image, switch between fit and actual size, and close with Escape. Previews use validated session references and require editor integration revision 2, included in the bundled core. Failed messages can retry with the same ID or return to the draft. Context actions capture selected code and diagnostics. Approval revalidates disk content; built-in edits refuse to overwrite known unsaved editor documents. Closing a view leaves the host session connected.
