...
def test_hook_registry_register_list_and_unregister() -> None:
    ...
    registry.register(HookPoint.BEFORE_TOOL_EXECUTE, AllowGuard(name="allow", priority=1))

    assert registry.list_hooks(HookPoint.BEFORE_TOOL_EXECUTE) == {"before_tool_execute": ["allow"]}

    registry.unregister(HookPoint.BEFORE_TOOL_EXECUTE, "allow")
    assert registry.list_hooks(HookPoint.BEFORE_TOOL_EXECUTE) == {"before_tool_execute": []}


def test_hook_registry_run_guards_stops_on_deny() -> None:
    registry = HookRegistry()
    registry.register(HookPoint.BEFORE_TOOL_EXECUTE, AllowGuard(name="allow", priority=10))
    registry.register(HookPoint.BEFORE_TOOL_EXECUTE, DenyGuard(name="deny", priority=5))
    registry.register(HookPoint.BEFORE_TOOL_EXECUTE, AllowGuard(name="later", priority=1))

    decisions = registry.run_guards(HookPoint.BEFORE_TOOL_EXECUTE, HookContext(hook_point=HookPoint.BEFORE_TOOL_EXECUTE))
...
def test_hook_registry_run_transforms_applies_priority_order() -> None:
    registry = HookRegistry()
    registry.register(HookPoint.AFTER_TOOL_EXECUTE, MetadataTransform(name="first", priority=10, key="a", value="1"))
    registry.register(HookPoint.AFTER_TOOL_EXECUTE, MetadataTransform(name="second", priority=5, key="b", value="2"))
...
def test_hook_registry_run_transforms_rejects_none_result() -> None:
    ...
    with pytest.raises(TypeError):
        registry.run_transforms(HookPoint.AFTER_TOOL_EXECUTE, HookContext(hook_point=HookPoint.AFTER_TOOL_EXECUTE))
...
def test_hook_registry_run_observers_fail_open() -> None:
    ...
    registry.register(HookPoint.AFTER_LLM_RESPONSE, RecordingObserver(name="good", bucket=bucket))

    registry.run_observers(HookPoint.AFTER_LLM_RESPONSE, HookContext(hook_point=HookPoint.AFTER_LLM_RESPONSE))
...
def test_hook_registry_clone_is_isolated_copy() -> None:
    ...
    assert registry.list_hooks(HookPoint.BEFORE_TOOL_EXECUTE) == {"before_tool_execute": ["allow"]}
    assert cloned.list_hooks(HookPoint.BEFORE_TOOL_EXECUTE) == {"before_tool_execute": []}
