...
def test_approval_engine_prefers_more_specific_rule() -> None:
    config = ApprovalConfig(
        ...
        rules=[
            ...
            ApprovalRuleConfig(tool_source="mcp", mcp_server="server-1", tool_name="search", action="allow"),
        ],
    )
    engine = ApprovalPolicyEngine(config)

    match = engine.evaluate(_ctx(tool_name="search", tool_source="mcp", mcp_server="server-1"))
...
def test_approval_engine_matches_profile_and_effect_class() -> None:
    ...
    default_match = engine.evaluate(_ctx(effect_class="filesystem_read", profile="coder"))
...
def test_approval_engine_specificity_scoring_orders_narrower_rules_higher() -> None:
    ...
    tool = ApprovalRuleConfig(tool_source="mcp", mcp_server="s1", tool_name="search", action="allow")

    assert ApprovalPolicyEngine._specificity(generic) < ApprovalPolicyEngine._specificity(server)
    assert ApprovalPolicyEngine._specificity(server) < ApprovalPolicyEngine._specificity(tool)
